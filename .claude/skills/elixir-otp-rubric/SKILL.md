...
Under real concurrency, app-level checks race; the **database is the source of truth for invariants**. Where the durable state is *files* rather than a DB, the same standard applies to file I/O.
- **File-handle leak safety:** every open is callback-form (`File.open!(path, modes, fun)` — closed on raise) or a `Stream.resource/3` whose `after_fun` closes on completion, halt, *and* exception. A bare `File.open` + later `close` leaks on any raise or early stream termination (e.g. client disconnect).
- **Atomic output:** writing a file in place truncates it immediately — a mid-write failure leaves a corrupt/partial file. Write to a temp file in the same directory, then `File.rename/2` (atomic on POSIX). Multi-file outputs (segments, bundles) need a stated policy for partial failure.
- **TOCTOU on stat-then-read:** any design that records `{path, offset, length}` (or stat/mtime) and reads later must state its immutability contract — the file can change between. Detect (size/mtime re-check, content fingerprint) or document the assumption; never silently serve stale bytes.
...
- **Flag:** check-then-act races (uniqueness, balance, allocation) with no DB constraint; `count(*)+1` or read-modify-write counters; multi-step writes outside `Ecto.Multi`; contended rows with no lock strategy; retryable work with no idempotency key; float money; bare `File.open` without raise-safe close; in-place file writes with no temp+rename; stat-then-read with no staleness story.

## 12. Security: multi-tenant isolation and untrusted input
In a multi-tenant SaaS, isolation and authorization are correctness; the default must be **deny**. In a library or service that decodes external data (uploads, media files, wire formats), *the input itself is the attacker* — resource bounds are correctness too. The tenant items below are N/A for a pure library; the untrusted-input items never are.
...
- **Untrusted-input decoders (binary/file/wire formats): validate declared sizes and counts against the bytes actually present *before* allocating or expanding.** The canonical bombs: `List.duplicate(x, count)` / run-length expansion / `1..count` loops where `count` is a raw field from the input (a 20-byte payload claiming `count = 0xFFFFFFFF` → multi-GB heap); a length-prefixed loop whose per-item width can be **zero bytes** (N iterations from no input — cap or reject); validation that runs only *after* full materialization (`length(list) != declared` is too late). Check `count * entry_width <= byte_size(rest)` first; binary pattern matches that fail on short input are fail-closed and fine. Also bound **algorithmic** cost driven by attacker-controlled dimensions (an O(n×m) expansion over two input-declared counts is a CPU bomb with no allocation).
- Parsers fail **closed** on malformed input: a lying size/offset field is rejected at parse time, not deferred to a raise mid-response after headers are sent. Eager and lazy/streaming parse paths must enforce the *same* bounds (a lazy path that skips payload reads must still check declared size against file size).
...
- **Flag:** any query not tenant-scoped; trusting a client `org_id`/`*_id` without an ownership re-check (IDOR); authz only in UI/mount and not the context; `cast` allowing role/owner/price/org fields; `String.to_atom` on external input; allocation/expansion sized by an input-declared count with no bytes-available check; a decode loop that can consume zero bytes per iteration; validation after materialization instead of before; O(n×m) work over attacker-controlled dimensions; lazy/streaming parse with weaker bounds than the eager path; secrets in repo/config or logged; PII/customer data to an LLM/third party without redaction+consent; outbound URL from user input with no SSRF guard.

## Severity calibration (drives the convergence loop and the audit `severity`)
- **Critical** — data loss/corruption, a security or tenant-isolation breach, unbounded memory/CPU driven by untrusted input (allocation bombs), a migration that locks a populated table, a race that double-charges, or anything that blocks implementation. Always loop.
